...
fn render_date(_state: &LongDescState, date: &WdDate, jusque: bool) -> String {
    let (_year, year_str, month, day) = match parse_time(&date.time) {
        Some(t) => t,
...
fn add_first_sentence(
    ...
    for (k, country_q) in nationalities.iter().enumerate() {
        let country_label = wd
            ...
            .unwrap_or_default();
        // For French, we use the country label directly (nationality transformation if available)
        if k > 0 {
            state.push_text("-");
        }
        let nat = if k > 0 {
...
fn push_date_range_fr(state: &mut LongDescState, item: &DatedItem) {
    if let Some(ref from) = item.date_from {
        // render_date already adds "en "/"le " prefix, so no extra word needed
        state.push_text(&render_date(state, from, false));
        state.push_text(" ");
    }
    if let Some(ref to) = item.date_to {
        // render_date with jusque=true adds "jusqu'en "/"jusqu'au " prefix
        state.push_text(&render_date(state, to, true));
        state.push_text(" ");
    }
}
